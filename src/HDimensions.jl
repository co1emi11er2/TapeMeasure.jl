"""
    mutable struct TopDimensions{T, S}
...
function dim_top(xs::Vector{T}, ys::Vector{S}; offset=zero(S)) where {T, S}
    ...
    if offset >= zero(S)
        return HDimensions(x_dims, y_dims, labels, minor_lines, major_lines)
    else
        return HDimensions(x_dims, y_dims, labels, major_lines, minor_lines)
    end
end
